...
base_dir = 'src/views/coffee-bean'

beans = [
    {'name': 'KenyaAADetail', 'data': {
        'id': 2, 'name': '肯尼亚AA', 'origin': '肯尼亚', 'category': '咖啡豆', 'rating': 4.8, 'reviews': 286,
    ...
    {'name': 'KenyaNeriDetail', 'data': {
        'id': 5, 'name': '肯尼亚 AA 涅里', 'origin': '肯尼亚', 'category': '咖啡豆', 'rating': 4.9, 'reviews': 198,
    ...
    {'name': 'HuilanDetail', 'data': {
        'id': 2, 'name': '哥伦比亚 蕙兰 玫瑰谷', 'origin': '哥伦比亚', 'category': '咖啡豆', 'rating': 4.6, 'reviews': 156,
    ...
    {'name': 'SantosDetail', 'data': {
        'id': 3, 'name': '巴西 桑托斯 半日晒', 'origin': '巴西', 'category': '咖啡豆', 'rating': 4.5, 'reviews': 234,
    ...
    {'name': 'AntiguaDetail', 'data': {
        'id': 4, 'name': '危地马拉 安提瓜 火山豆', 'origin': '危地马拉', 'category': '咖啡豆', 'rating': 4.8, 'reviews': 178,
    ...
    {'name': 'MandarinDetail', 'data': {
        'id': 9, 'name': '印尼 苏门答腊 曼特宁', 'origin': '印尼', 'category': '咖啡豆', 'rating': 4.7, 'reviews': 212,
    ...
    {'name': 'CostaRicaDetail', 'data': {
        'id': 7, 'name': '哥斯达黎加 蜜处理 黑蜜', 'origin': '哥斯达黎加', 'category': '咖啡豆', 'rating': 4.6, 'reviews': 145,
...
with open(os.path.join(base_dir, 'YirgacheffeDetail.vue'), 'r', encoding='utf-8') as f:
    template = f.read()

for bean in beans:
    data = bean['data']
    data_lines = ['  id: ' + str(data['id'])]
    for key, val in data.items():
        if key == 'id':
            continue
        if isinstance(val, str):
            data_lines.append('  ' + key + ": '" + val + "'")
        elif isinstance(val, list):
            items = ', '.join("'" + v + "'" for v in val)
            data_lines.append('  ' + key + ': [' + items + ']')
        elif isinstance(val, dict):
            dict_items = ', '.join("'" + k + "': " + str(v) for k, v in val.items())
            data_lines.append('  ' + key + ': {' + dict_items + '}')
        else:
            data_lines.append('  ' + key + ': ' + str(val))
    
    data_str = 'const productData = {\n' + ',\n'.join(data_lines) + '\n}'
    content = template.replace('const productData = {\n  id: 1,\n  name: \'耶加雪菲 果丁丁\',', data_str[:-1])
    content = content.replace('},', '}')
    
    default_spec = data['specs'][0]
    content = content.replace("selectedWeight = ref('100g')", "selectedWeight = ref('" + default_spec + "')")
    
    with open(os.path.join(base_dir, bean['name'] + '.vue'), 'w', encoding='utf-8') as f:
        f.write(content)
    
    print('Created: ' + bean['name'] + '.vue')

print('All 7 components created successfully!')